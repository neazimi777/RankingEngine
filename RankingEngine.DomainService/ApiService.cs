...
public class GenericApiClient
{
    ...
    public GenericApiClient(HttpClient http, ILogger<GenericApiClient> logger)
    {
        _http = http;
        _logger = logger;
    }
    ...
    public async Task<ApiResponseDto<T>> SendAsync<T>(ApiRequestDto apiRequestDto, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogDebug("Sending {Method} {Url}", method, finalUrl);
